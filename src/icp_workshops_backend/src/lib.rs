...
async fn translate(text: String) -> Result<String, String> {
    ...
    ).await.map_err(|error| format!("Error while quering data. Status: {:?}, Error: {}", error.0, error.1))?;


    let formated_res: (Response,) = serde_json::from_slice(&res.0.body).map_err(|error| format!("Error while quering data. Error: {}", error))?;
